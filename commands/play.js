...
module.exports = {
  data: new SlashCommandBuilder()
    ...
    .addSubcommand((subcommand) =>
      subcommand
        .setName("spotysong")
        .setDescription("Plays a single song from Spoty")
        .addStringOption((option) =>
          option
            .setName("url")
            .setDescription("the song's url")
    ...
    .addSubcommand((subcommand) =>
      subcommand
        .setName("spotyplaylist")
        .setDescription("Plays a playlist from Spoty")
        .addStringOption((option) =>
          option
            .setName("url")
            .setDescription("the song's url")
  ...
  execute: async ({ client, interaction }) => {
    ...
    if (interaction.options.getSubcommand() === "song") {
      ...
      embed
        ...
        .setThumbnail(song.thumbnail)
        .setFooter({ text: `Duration: ${song.duration}` });
    } else if (interaction.options.getSubcommand() === "spotysong") {
      let url = interaction.options.getString("url");
      ...
      embed
        ...
        .setThumbnail(song.thumbnail)
        .setFooter({ text: `Duration: ${song.duration}` });
    } else if (interaction.options.getSubcommand() === "spotyplaylist") {
      // Search for the playlist using the discord-player
      let url = interaction.options.getString("url");
      const result = await client.player.search(url, {
        requestedBy: interaction.user,
